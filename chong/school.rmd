...
students     <- fread("pone.0153690.s001.csv") #678
interactions <- fread("pone.0153690.s003.csv") #10781
```
...
fit3=ergm(net2 ~ edges  + nodematch('gender')+ nodefactor('grade')+nodematch('grade') +nodematch('lunch'))

fit4= ergm(net2 ~ edges  + nodematch('gender')+ nodefactor('grade')+nodematch('grade') +nodematch('lunch')+altkstar(0.5, fixed=TRUE))

fit3r=ergm(net2 ~ edges  + nodematch('gender')+ nodefactor('grade')+nodematch('grade') +nodematch('lunch')  +offset(degree(0)) , offset.coef = -Inf)

saveRDS(fit3, file = "~/school/the_model.rds")
#mcmc.diagnostics(fit3r)
```

```{r model-tabel, echo = FALSE, results='asis'}
texreg::knitreg(list(fit1, fit2, fit3, fit4, fit3r))
```

Results are almost identical with or without adding offset of isolates, so we use model 3 as our final model.

## Simulate 1000 networks using model 3. 
Get the averaged network summaries of the 1000 simulated networks, and compare with the real network data.
```{r }
sim1000=simulate(fit3, nsim=1000, seed=1234)
sumsim=colMeans(attr(sim1000, 'stats'))
sumnet=summary(net2 ~  edges +nodematch('gender')+nodefactor('grade')+nodematch('grade')+nodematch('lunch'))
data.table(Terms=names(sumsim), observed=sumnet, simulated=sumsim)
```

Looks good. Save the simulated 1000 networks
```{r}
saveRDS(sim1000,  file = "~/school/Simulated_1000_networks.rds")
```
